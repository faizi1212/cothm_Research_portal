...
const storage = new CloudinaryStorage({
    ...
    params: { folder: 'cothm-enterprise', resource_type: 'auto', allowed_formats: ['pdf', 'doc', 'docx', 'jpg', 'png'] }
});
...
const UserSchema = new mongoose.Schema({
    ...
    avatar: { type: String, default: "" }, // New: Profile Pic
    role: { type: String, default: "student" },
    lastLogin: { type: Date, default: Date.now }, // New: Analytics
...
const NotificationSchema = new mongoose.Schema({
    recipient: String, message: String, type: { type: String, default: 'info' }, read: { type: Boolean, default: false }, date: { type: Date, default: Date.now }
});
const Notification = mongoose.model('Notification', NotificationSchema);

const AnnouncementSchema = new mongoose.Schema({ title: String, message: String, priority: String, date: { type: Date, default: Date.now } });
...
app.post('/api/auth/login', async (req, res) => {
    try {
        ...
        user.lastLogin = new Date(); // Update analytics
        await user.save();
...
app.post('/api/submit', upload.single("file"), async (req, res) => {
    ...
    await project.save();
    await Notification.create({ recipient: 'admin', message: `📄 New submission: ${studentName} (${stage})`, type: 'info' });
